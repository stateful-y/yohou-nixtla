...
```bash
git clone https://github.com/YOUR_USERNAME/yohou-nixtla.git
cd yohou-nixtla
```
...
Yohou-Nixtla uses pytest with markers to categorize tests into different types:
...
=== "uv run"
    ...
    ```bash
    uv run pytest --cov=yohou_nixtla --cov-report=html
    ```
...
Fully qualified names work too (`yohou_nixtla.module.OtherClass`), and
...
In this notebook, we will build a small Yohou-Nixtla pipeline end to end
and inspect what it produces.

**Prerequisites:** Python 3.11+ and basic familiarity with yohou_nixtla.
```
...
This notebook shows how to configure yohou_nixtla to drop incomplete
...
- **API cross-links**: When mentioning yohou_nixtla classes or functions in markdown cells, wrap them in backtick-link syntax pointing to the API page.
...
- [Open an issue on GitHub](https://github.com/stateful-y/yohou-nixtla/issues/new)
- [Start a discussion in the repository](https://github.com/stateful-y/yohou-nixtla/discussions)
